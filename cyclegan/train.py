...
def train_fn(disc_H, disc_Z, gen_Z, gen_H, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler):
    ...
    for idx,  (zebra, horse) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            # Adverserial loss for both generators
            D_H_fake = disc_H(fake_horse)
            D_Z_fake = disc_Z(fake_zebra) # Without detatch()
...
def main():
    ...
    opt_gen = optim.Adam(list(gen_H.parameters()) + list(gen_Z.parameters()), lr=config.LEARNING_RATE, betas=(0.5, 0.999))
    
    L1 = nn.L1Loss() # cycle consistency and identity loss
